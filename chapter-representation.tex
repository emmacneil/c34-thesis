...
\note{This proposition depends on 7.3 below, so some reorganization is needed.
Prop 7.3 does not depend on this.}
...
\begin{proof}
  ...
  For uniqueness, suppose $A \equiv B$ are equivalent reduced divisors.
...
\begin{center}
  ...
  \end{tabular}
\end{center}
\caption{Divisor types and the degree of their flips}
\end{table}
...
The next several lemmas build up to that claim.
...
\begin{proof}
  ...
  \note{(This argument needs to be adjusted for when this is a double point)}
...
\note{
In the course of deriving explicit formulae,
we occasionally make use of identities or dependencies between the coefficients of a type 31 divisor,
or other properties of type 31 divisors.
This section will record these identities.
}

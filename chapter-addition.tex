...
\begin{center}
\begin{tikzcd}
  W_{D + D'}^{x^2y} \arrow[hook]{r}{\ker M} &
  W_D^{x^2y} \arrow[hook]{r}{\iota} \arrow[bend left]{rr}{M} &
...
\note{Note to self that I need to define (in section \ref{sec_groebner_bases_in_coordinate_rings})
monomials in $K[C]$ and $\LM(f)$ for $f \in K[C]$.
This amounts to treating polys in $K[C]$ as polys in $K[x,y]$ with no $y^3$ term.}
...
\begin{proof}
  ...
  \begin{align*}
    \dim W_D^m
      ...
      &= \dim W^m - \deg D.
  \end{align*}
  \note{The last equality is a consequence of Corollary \ref{cor_dim_R_mod_I}.
  I need to make the relationship between that corollary and the degree of a divisor clear in Chapter 7.}
\end{proof}
...
We begin with a proposition characterizing the case $\rank M = 0$.
\begin{proposition}
  \label{prop_rank_M}
...
\note{If necessary, formulas for this can be placed in appendix.}
...
\note{Explicit formulas for tripling may also to be given in appendix. Or next chapter?}

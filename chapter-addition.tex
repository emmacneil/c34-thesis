...
they first identify these divisors with vector spaces $W_D^{x^2y}$ and $W_{D'}^{x^2y}$.
...
\begin{enumerate}[label=(\roman*)]
  \item when $D$ and $D'$ are non-disjoint,
        the kernel is in fact computing $L = \lcm(D, D')$;
  \item $G = \gcd(D, D')$ may be computed by computing the image of the quotient; and
  \item the monomial $x^2y$ indicates an upper bound on a search space ---
        ...
        $D + D'$ is atypical.
\end{enumerate}
The generalization of Abu Salem and Khuri-Makdisi's addition requires
computing the kernel and image of $M$ in
\begin{center}
\begin{tikzcd}
